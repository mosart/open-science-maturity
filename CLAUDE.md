...
- `download_pdf()` renders `document.body` via `jsPDF` (loaded from a CDN, `doc.html()`) into `report.pdf`. Because `doc.html()` renders via html2canvas using the page's on-screen computed styles, it does not fire `beforeprint`/honor `@media print`, so `download_pdf()` does its own force-open of every `<details>` and toggles a plain `body.printing` class (mirrored by a non-media-query CSS rule) to hide the sidebar footer's buttons, the export/import accordion, and unselected status cards, restoring the original `<details>` open/closed state and removing the class once the PDF is saved. The browser's own print preview (Ctrl/Cmd+P) instead uses the `@media print` rules directly, with `beforeprint`/`afterprint` listeners that force-open all `<details>` and restore their prior state afterward.
