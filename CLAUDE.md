...
- `download_pdf()` renders `document.body` via `jsPDF` (loaded from a CDN, `doc.html()`) into `report.pdf`. `@media print` rules hide interactive-only chrome (the actions row, badge download buttons); the `beforeprint` listener force-opens all `<details>` so every principle's chosen status and notes are visible in the printed/PDF view, not just the titles. Note: jsPDF's `doc.html()` renders via html2canvas using the page's on-screen computed styles — it is not confirmed whether it reliably honors `@media print`/fires `beforeprint` the same way the browser's own print preview does. This should be manually verified in a browser (compare "Download report" output against Ctrl/Cmd+P print preview) before relying on this description as fact.
